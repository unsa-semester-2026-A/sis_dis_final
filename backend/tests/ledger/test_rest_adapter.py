...
# Reuse the in-memory repos from test_use_cases
...
class TestCreateNodeEndpoint:
    def test_create_asset_node(self, client: TestClient) -> None:
        user_id = str(new_id())
        resp = client.post("/ledger/nodes", json={
            "user_id": user_id,
            "name": "Main Bank",
            "node_type": "ASSET",
            "currency": "PEN",
...
class TestEmitVectorEndpoint:
    def _create_node(
        self, client: TestClient, name: str, node_type: str
    ) -> str:
        resp = client.post("/ledger/nodes", json={
            "user_id": str(new_id()),
            "name": name,
    ...
    def test_emit_vector(self, client: TestClient) -> None:
        ...
        resp = client.post("/ledger/vectors", json={
...
class TestEvaluateBalanceEndpoint:
    def test_balance_after_operations(self, client: TestClient) -> None:
        user_id = str(new_id())
        # Create nodes
        bank_resp = client.post("/ledger/nodes", json={
            "user_id": user_id, "name": "Bank",
            "node_type": "ASSET", "currency": "PEN",
        })
        salary_resp = client.post("/ledger/nodes", json={
            "user_id": user_id, "name": "Salary",
            "node_type": "SOURCE", "currency": "PEN",
        })
        food_resp = client.post("/ledger/nodes", json={
            "user_id": user_id, "name": "Food",
            "node_type": "SINK", "currency": "PEN",
        })
        ...
        # Salary -> Bank: 1000
        client.post("/ledger/vectors", json={
